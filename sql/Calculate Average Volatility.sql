SELECT
  AVG(view_cart_change) AS avg_view_to_cart_volatility,
  AVG(cart_checkout_change) AS avg_cart_to_checkout_volatility,
  AVG(checkout_purchase_change) AS avg_checkout_to_purchase_volatility
FROM (

  SELECT
    event_dt,
    ABS(view_to_cart_rate -
        LAG(view_to_cart_rate) OVER (ORDER BY event_dt)) AS view_cart_change,

    ABS(cart_to_checkout_rate -
        LAG(cart_to_checkout_rate) OVER (ORDER BY event_dt)) AS cart_checkout_change,

    ABS(checkout_to_purchase_rate -
        LAG(checkout_to_purchase_rate) OVER (ORDER BY event_dt)) AS checkout_purchase_change
