select event_dt, total_revenue,
  lag(total_revenue) over (order by event_dt) as prev_day_revenue,
  total_revenue - lag(total_revenue) over (order by event_dt) as revenue_change
from (--Daily Funnel Aggregation
with base as (
  select 
